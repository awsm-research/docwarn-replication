...
def runForProjectList():
    ...
    for project in projects.infoChgList:
        ...
        revertedDf[field.TEXT] = revertedDf[field.TEXT].apply(lambda x: util_logic.cleanJiraString(x, forBert=True, doLower=False))
...
def runBert(trainDf, testDf, project, roundFileName, randomRound, cvRound):
    ...
    revertedDf = util.read_csv_pandas(paths.data + paths.dataRevertedCleaned + project.key)
    revertedDf = util.fillNanWithEmptyString(revertedDf)
    trainDf, testDf = util_logic.prepareBertText(revertedDf, trainDf, testDf, doLower=True)
    ...
    model = tf.keras.Model(inputs=[input_ids_layer, input_attention_layer], outputs=output)
    ...
    train_history1 = model.fit(
        ...
        validation_data=([X_valid_ids, X_valid_attention], validDf[field.LABEL].to_numpy()),
        verbose=1,
        callbacks=[overfitCallback]
    )

    trainDf['predicted'] = model.predict([X_train_ids, X_train_attention])
    threshold = util_logic.getDynamicThreshold(trainDf, field.LABEL)
    y_test_predicted = model.predict([X_test_ids, X_test_attention])
